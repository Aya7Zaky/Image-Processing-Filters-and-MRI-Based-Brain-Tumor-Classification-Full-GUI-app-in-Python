...
####
    ...
    def classification(self):
        dir = 'C:\\Users\\star\\Downloads\\archive\\brain_dataset'
        classes = ['yes','no']
        ...
        # training will happen if the trained model isnot available
                ...
                # once the model hasbeen trained ,save it into file :
                with open("train_model.pickle","wb") as file:
                    pickle.dump(model,file)
     # Make prediction 
        self.new_image=self.image_rgb
        self.new_image = cv2.resize(self.new_image,(100,100))
        self.new_image = (np.array(self.new_image).flatten())/255
        pred=model.predict([self.new_image])
        from sklearn import metrics
        predict = model.predict(X_test)
        acc = metrics.accuracy_score(Y_test,predict)*100 
        
        messagebox.showinfo("showinfo", "Prediction is:[ "+classes[int(pred)]+" ]\nAccurecy = "+str(acc))
    ...
    def create_widgets(self):
        ...
        self.canvas1.configure(width=900,height=480, bg='gray15',highlightbackground="Deep Sky Blue3",highlightthickness=1) 
        self.canvas1.grid(column=0, row=1)
       ...
       #Frame1
        ...
        self.frame1_button.grid(padx=20, pady=50)
      ...
      #Frame2
        ...
        self.frame2_button.grid(padx=20, pady=50) 
        ...
        self.button_cls.config( text='  Classification   ')
